...
Ground Shipping - This shipping option has a small flat charge plus a rate based on the weight of your package.
Ground Shipping Premium - This shipping option does not charge for weight, but has a higher flat charge.
Drone Shipping - This shipping option has no flat charge, but the rate based on weight is triple the rate of ground shipping. 
...
def greet_customer():
    """
    Asks the customer to enter their name as part of welcoming them Sal's Shippers.
    ...
    while True:
        ...
        if name != "":
            ...
            print("\nWe take pride in ensuring that our customers get the best, and most affordable experience shipping their packages.")
            return name
...
def validate_user_input(prompt_text):
    """
    Checks the user input for a ValueError.
...
def get_package_weight():
    ...
    A warning message will be displayed to the customer if the customer did not enter anything into the prompt.
...
def calculate_ground_shipping_cost(weight):
    """
    Calculates the shipping cost of using Ground Shipping.
    """ 
...
def calculate_drone_shipping_cost(weight):
    """
    Calculates the shipping cost of using Drone Shipping.
    """
...
def print_cheapest_shipping_option(total_package_weight, consumer_name):
    """
    Finds the cheapest shipping option for the customer.
    Prints out a message to inform the customer that the program has found the best and affordable shipping method for them.
    Prints out the weight of the package.
    Prints out a message that tells the customer which shipping method is the cheapest.
    """
    ...
    print(f"\n\nGood news, {consumer_name}, we have found the perfect shipping method for you based on your package's weight.")
    ...
    elif (PREMIUM_GROUND_SHIP < ground_ship) and (PREMIUM_GROUND_SHIP < drone_ship):
...
def main():
    """
    Calls greet_customer function to greet the customer when the program first executes.

    Asks the customer to enter the weight of their package and finds the cheapest shipping option for them.
    
    Prompts the customer to enter their package's weight again if the customer has another package to ship and wants
    to check which shipping method is the cheapest based on the package's weight.
    ...
    while True:
        ...
        if not ask_user_yes_no("\n\nDo you have another package to weight? (Y/N): "):
            print("\n\nExiting Program...\n\n")
            exit()
