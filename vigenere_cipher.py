...
def get_mode(text, key):
    """
    Asks the user whether they want to encrypt or decrypt their message.
    Return statement calls encode_message function or decode_message function depends on the mode that the user chose
    and translate the message based on the chosen mode. 
    """

    while True:
        mode = input("\n\nDo you wish to encrypt or decrypt the message: ").lower()
...
def get_keyword():
    """
    Asks the user to enter a keyword that is used to shift the value of each letter in the alphabets
    and generate a key phrase that has the same length as the message.
...
def check_text_case(symbol: str, result: list, key_position: int) -> list:
    """
    Checks whether the letter in the message is an uppercase or a lowercase in order to
    get the translated message to match the casing of the original message.  
    ...
    Appends the encrypted (or decrypted) character to result using key_position as the index 
    to match the position of the letter in ascii_letters
    """
...
def check_index_length(index, key):
    """
    Checks if the index that is used to match the length of the given keyword is equal to the
    length of the keyword.
    Resets index to 0 if index is equal to the length of the keyword and returns an integer of 0
...
def vigenere_translate(message, key, mode):
    """
    Builds the encrypted (or decrypted) string one character at a time and store 
    these characters in translated_message.

    The key_index variable keeps track of which subkey to use since a different key is used 
    depending on the position of the letter in the message.
    Sets key_index to 0 since the letter used to encrypt or decrypt the first letter of the message will be 
    the one at key[0] where key variable is the keyword given by the user.

    The key_index (subkey) will always be what key[key_index] evaluates to.
    For instance: key_index is 1, keyword is "dog", key[key_index] or dog[1] evaluates to the letter 'o'.

    Sets the characters in message to the variable char on each iteration of the for loop.

    Finds the index of char in ascii_letters and assigns the result to char_position.

    The letter in char is found in ascii_letters if char_position is not set to -1.

    Checks if the mode chose by the user is encryption or decryption. 
    += is used for encryption and -= is used for decryption.
    Mods the integer stored in char_position if the new value of char_position is less than 0 or if the new
    value of char_position is the length of ascii_letters or greater.

    Increments key_index by 1 after char has been translated in order to use the next subkey on the 
    next iteration of the for loop.
    This way when the next iteration uses key[key_index] to get the subkey, it will be the index to the next subkey.

    Appends the character untranslated if char was not found in ascii_letters. E.g. char is a number 
    or puncutuation marks such as '10', '?', or ':'.

    After building the string from each character and appending it to translated_message, calls print_translated_message
    and join method on the blank string (the result of appending untranslated character) to join together all 
    the strings in translated_message.
    """
...
def should_translate_again():
    """
    Asks the user to enter a message and give a secret keyword in order to encrypt the message. Decrypting uses
    the same keyword that is used to encrypt the message.

    Calls get_mode to check whether the user wants to encrypt or decrypt their message and translates the message 
